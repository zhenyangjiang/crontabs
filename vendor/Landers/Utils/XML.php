...
class XML
{
     ...
     * @return array
     */
    public static function parse($xml, $retRoot = false)
    {
        $xml = simplexml_load_string($xml, 'SimpleXMLElement', LIBXML_COMPACT | LIBXML_NOCDATA | LIBXML_NOBLANKS);
        if (is_object($xml) && get_class($xml) === 'SimpleXMLElement') {
            $root = $xml->getName();
            $data = self::arrarval($xml);
            return !$retRoot ? $data : array('data' => $data, 'root' => $root);
        } else {
            return NULL;
        }
    }
